...
# JOIN 3 Tables
    ...
    def get_hospital_with_doc_disease(cls,data):
        query= "SELECT * FROM hospitals join doctors on doctors.hospital_id=hospitals.id join diseases on diseases.doctor_id=doctors.id join categories on categories.id=diseases.category_id where hospitals.id=%(hos_id)s and categories.id =%(cat_id)s;"
        return connectToMySQL(cls.db_name).query_db(query,data)
